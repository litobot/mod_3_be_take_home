...
RSpec.describe "Index Action" do
  describe "GET /api/v1/tea_subscriptions" do
    ...
    it "returns all tea subscriptions with correct JSON format" do
      ...
      expect(tea_subscription['type']).to eq('tea_subscription')

      attributes = tea_subscription['attributes']
      expect(attributes).to have_key('tea_title')
      expect(attributes).to have_key('subscription_frequency')
      expect(attributes).to have_key('customer_name')
      expect(attributes).to have_key('subscription_status')

      expect(attributes['tea_title']).to eq('Green Tea')
      expect(attributes['subscription_frequency']).to eq('monthly')
      expect(attributes['customer_name']).to eq('John Doe')
